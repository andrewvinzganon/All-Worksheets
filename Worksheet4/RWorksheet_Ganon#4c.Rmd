...
car_counted
```
# a. Plot using geom_bar() using the top 20 observations only. The graphs shoudl have a title, labels and colors. 
```{r}
...
ggplot(car_counted, aes(x = model, y = count)) +
  ...
  coord_flip()
```
#5. Plot the relationship between cyl - number of cylinders and displ - engine displacement using geom_point with aesthetic color = engine displacement. Title should be “Relationship between No. of Cylinders and Engine Displacement”. a. How would you describe its relationship? 
